package com.example.appporcino.ui.ganado;

import android.os.Bundle;
import android.view.LayoutInflater;
import android.view.MenuItem;
import android.view.View;
import android.view.ViewGroup;
import android.widget.TextView;

import androidx.annotation.Nullable;
import androidx.annotation.NonNull;
import androidx.fragment.app.Fragment;
import androidx.lifecycle.Observer;
import androidx.lifecycle.ViewModelProviders;

import com.example.appporcino.R;
import com.example.appporcino.ui.Agregar.AgregarFragment;
import com.example.appporcino.ui.alertas.AlertasFragment;
import com.example.appporcino.ui.gastos.GastosFragment;
import com.google.android.material.bottomnavigation.BottomNavigationView;
import com.google.android.material.bottomnavigation.LabelVisibilityMode;

public class GanadoFragment extends Fragment {

    private GanadoViewModel ganadoViewModel;
    public View onCreateView(@NonNull LayoutInflater inflater,
                             ViewGroup container, Bundle savedInstanceState) {
        ganadoViewModel =
                ViewModelProviders.of(this).get(GanadoViewModel.class);
        View root = inflater.inflate(R.layout.fragment_ganado, container, false);
        final TextView textView = root.findViewById(R.id.text_ganado);
        ganadoViewModel.getText().observe(this, new Observer<String>() {
            @Override
            public void onChanged(@Nullable String s) {
                textView.setText(s);
            }
        });
        return root;
    }
}